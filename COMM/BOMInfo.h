...
#define theBOM_INFO CBomInfo::GetInstance()

class COMMON_API CBomInfo
{
public:

	static CBomInfo&  GetInstance();
	...
	int GetReceiptPrintTimes(CString receiptName);	//凭条打印次数
...
private:
	static CBomInfo theInstance;
	CGetIni* m_bomIni;

	// 隐藏构造函数
	CBomInfo();
	~CBomInfo();
	CBomInfo(const CBomInfo&);
	CBomInfo& operator=(const CBomInfo&);
