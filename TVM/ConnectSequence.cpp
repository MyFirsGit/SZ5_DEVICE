...
bool CConnectSequence::CanAcceptSCCommand(CSCCommand* command)
{
	...
	if(m_status == PARAMETERSYNCED &&  theSERVICE_MGR.GetService<CServerClientSvc>(SC_SVC)->GetCommandMsgCode(command)==MSG_DEVICE_ALL_STATUS_DATA)
	{
		return true;
	}
	if(m_status == MODEUPLOADED)
	{
		WORD availableMsgCode[] = {MSG_DEVICE_STATUS_CHANGE_DATA,MSG_OPERATION_DATA,MSG_24HOUR_OPERATION_DATA,MSG_DELAY_OPERATION_DATA,MSG_OP_END_NOTIC};
...
void CConnectSequence::OnSCCommandComplete(CSCCommand* command)
{
	...
	if(m_status == AUTHED )
	{
		...
		if(parameterSyncSequence->IsCompleted())
		{
			...
			theSERVICE_MGR.GetService(SC_SVC)->SendMessage(SC_MSG_SEND_DEVICE_FULLSTATUS,STATUS_TYPE_THREESTATUS,NULL);
		}
		return;
	}
	if(m_status == PARAMETERSYNCED &&  theSERVICE_MGR.GetService<CServerClientSvc>(SC_SVC)->GetCommandMsgCode(command) ==MSG_DEVICE_ALL_STATUS_DATA)
