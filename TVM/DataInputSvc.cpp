...
BEGIN_MESSAGE_MAP(CDataInputSvc, CTVMForeService)
...
CDataInputSvc::CDataInputSvc()
	:CTVMForeService(DATA_INPUT_SVC)
{
	//m_ErrorCode = -1;
	// 设置操作员画面
	m_pDialogFlow->AddDialog(IDD_20121_DATAINPUT_SELECT_DLG,new CDataInputSelectMaintainDlg(this));
	m_pDialogFlow->AddDialogGroup(DIALOG_GROUP_DATAINPUT_SELECT,_opl(GUIDE_DATA_INPUT_MSGBOX_CONFIRM));
...
*/
//////////////////////////////////////////////////////////////////////////
bool CDataInputSvc::IsUSBAuthed(){
...
*/
//////////////////////////////////////////////////////////////////////////
LRESULT CDataInputSvc::OnKeyboardEnter(WPARAM , LPARAM )
{
	// 如果在线则提示不允许设置
	if(theTVM_STATUS_MGR.GetCommunicate() == CONNECTED){
		theAPP_SESSION.ShowOfficeGuide(_opl(TXT_WHEN_SC_CONNECTION_CANNOT_USE_THIS_FUNC));
		...
		//确认导入操作 
		theAPP_SESSION.ShowOfficeGuide(GUIDE_DATA_INPUT_DONING);
		theSERVICE_MGR.SetForeServiceBusy(true);
		m_bIsIndexFileExist ? ImportParameter() : theAPP_SESSION.ShowOfficeGuide(_opl(GUIDE_DATA_INPUT_ERROR_INDEX_EXIST));
...
*/
//////////////////////////////////////////////////////////////////////////
LRESULT CDataInputSvc::OnKeyboardF3(WPARAM , LPARAM ){
...
*/
//////////////////////////////////////////////////////////////////////////
void CDataInputSvc::NotifyDataModelChanged(){
...
*/
//////////////////////////////////////////////////////////////////////////
bool CDataInputSvc::IsIndexFileExist(){
	return m_bIsIndexFileExist;
}
...
void CDataInputSvc::InitDataModel()
{
	...
	CString *parameterNames = thePARAM_HELPER.GetParameterNames();
	typedef std::map<PARAMETER_ID,CParameterMaster*>::iterator iterator;
	int i = 0;
	TCHAR szRunPath[MAX_PATH];
	ComGetAppPath(szRunPath, MAX_PATH);

	for(iterator iter = mapVersionProvider.begin();iter!=mapVersionProvider.end();iter++,i++)
	{
		_tagParameterInputStruct &inputStruct = m_DataModel[iter->first];
		inputStruct.name = parameterNames[i];
		CString strID;
		strID.Format(_T("(%x)"), iter->first);
		inputStruct.name += strID;
		inputStruct.localCurrentVersion = iter->second->GetCurMasterVer();
		inputStruct.localFutureVersion = iter->second->GetFutMasterVer();
		inputStruct.importCurrentTo = CString(szRunPath) + PM_UPDATE_DIR;
		inputStruct.importFutureTo = CString(szRunPath) + PM_UPDATE_DIR;
	}
// 	AFC相关参数
...
// 		}
	...
	//	PARAMETER_ID parameterID = (PARAMETER_ID)ACC_STATION_MAP_ID;
	//	PARAM_VERSION_TYPE_CODE verCode = CUR_VER;
	//	inputStruct.importCurrentTo = theUPGRADE_HELPER::GetDownloadFolder(parameterID,verCode);
	//	verCode = FUT_VER;
	//	inputStruct.importFutureTo = theUPGRADE_HELPER::GetDownloadFolder(parameterID,verCode);
...
bool CDataInputSvc::ImportParameter()
{
	...
	for(DataItem iter = m_DataModel.begin();iter != m_DataModel.end();iter++)
	{
		...
		// 允许只导入某些文件或参数
		else if(inputStruct.importCurrentFrom.IsEmpty()){
			continue;
		}
		else
		{
...
{
	try{
		m_bIsAuthPass = uc_authenticate() == 1;
		return m_bIsAuthPass;
	}
