...
int CVersionInfo::GetCompanyID(VER_TYPE verType,PROG_TYPE progType)
{
	...
	CString sKey(_T(""));
	if(verType == CURRENT){
		sKey = "CURRENT_COMPANY_ID";
	}
	else if(verType == FUTURE){
		sKey = "FUTURE_COMPANY_ID";
	}
	else{
	}
	return m_versionIni->GetDataInt(GetSectionName(progType), sKey, 0);
...
CString CVersionInfo::GetSectionName(__in PROG_TYPE progTye)
{
	...
	switch(progTye){
		case MC:
			...
			break;
 		case TH_MAIN:
 			key = _T("CARD_MODULE_MAIN");
 			break;
 		case TH_APP:
 			key= _T("CARD_MODULE_APP");
 			break;
		case CARD_RW_MAIN:
			key = _T("CARD_RW_MAIN");
			break;
		case CARD_RW_APP:
			...
			break;
 		//case TOKEN_RW_MAIN:
 		//	key = _T("TOKEN_RW_MAIN");
 		//	break;
 		//case TOKEN_RW_APP:
 		//	key = _T("TOKEN_RW_APP");
 		//	break;
		case CHARGE_RW_MAIN:
			key = _T("RECHARGE_RW_MAIN");
			break;
		case CHARGE_RW_APP:
			key = _T("RECHARGE_RW_APP");
			break;
		//收据模板
		case TEMPLATE:
			key = _T("TEMPLATE_TYPE");
			break;
		//GUI文件
		case GUI:
			key = _T("GUI_TYPE");
...
@retval     CString strVersion(MC版本)

@exception  无
*/
//////////////////////////////////////////////////////////////////////////
CString CVersionInfo::GetMCVersionNoString(){
	CString strVersion;

	strVersion.Format(_T("%d"),GetVersionNo(CURRENT,CVersionInfo::MC));
