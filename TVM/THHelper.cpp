...
void CTHHelper::Initialize()
{
	...
	try {
		...
		// 设置版本
		if(theTVM_SETTING.GetTHMode() == TH_MODE_NORMAL && !theEXCEPTION_MGR.HasException(CTHHardwareException::MODULE_ID) && !theEXCEPTION_MGR.HasException(CTHException::MODULE_ID))
		{
			// 记录TH硬件版本
...
long CTHHelper::CommOpen()
{
	...
	cmd_comm_open.uiCommPort = theTVM_SETTING.GetTHComPort();
	cmd_comm_open.uiBaudRate = theTVM_SETTING.GetTHBaudRate();
 ...
 int CTHHelper::UpdateProgram(){
	...
	try
	{
		...
		for(int i=0;i<updateFiles.size();i++)
		{
			...
			THUpdateCommand cmdUpdate(theTVM_SETTING.GetTHComPort(),theTVM_SETTING.GetTHBaudRate(),theTVM_SETTING.GetTHUpadateBaudRate(),(char*)strFilePath.c_str());
