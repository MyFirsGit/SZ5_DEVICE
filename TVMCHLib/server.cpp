...
BOOL GetSendRequest(IN HTDLWORK* pWork, IN SENDITEM* pItem, OUT CHTRS_RESULT* pRes);
BOOL GetRecvRequest(IN HTDLWORK* pWork,	IN RECVITEM* pItem,	OUT CHTRS_RESULT* pRes);
...
BOOL InitComPort(IN PDEVICETBL pTbl)
{
	...
	DWORD nWait;
		...
		CHTrs_Trace("InitComPort - Enter");
	#endif

	pTbl->nResult = CHTRS_RES_WINERR;
	// 串口通信
	do{
		...
		// 打开串口
		if (pTbl->Parm.ComPort>=10)	{
			sprintf(buff, "COM%d", (pTbl->Parm.ComPort));	
		}
		else{
			sprintf(buff, "\\\\.\\COM%d", (pTbl->Parm.ComPort));
		}
		...
		if (pTbl->pWork->hComm == INVALID_HANDLE_VALUE){
			pTbl->nResult = CHTRS_HTDL_ERR_PORT;
			break;
		}
		// 设置输入输出缓存大小
		...
		if (FALSE == ::GetCommState(pTbl->pWork->hComm, &dcb)){
			#ifdef _TRACE_SERVER_
				CHTrs_Trace("InitComPort - GetCommState() = FALSE");
			#endif
			pTbl->nResult = CHTRS_RES_WINERR;
		...
		if (FALSE == ::SetCommState(pTbl->pWork->hComm, &dcb)){
			#ifdef _TRACE_SERVER_
				CHTrs_Trace("InitComPort - SetCommState() = FALSE");
			#endif
			pTbl->nResult = CHTRS_RES_WINERR;
			::CloseHandle(pTbl->pWork->hComm);
			break;
		...
		if (FALSE == ::SetCommTimeouts(pTbl->pWork->hComm, &timeout)){
			#ifdef _TRACE_SERVER_
				CHTrs_Trace("InitComPort - SetCommTimeouts() = FALSE");
			#endif
			pTbl->nResult = CHTRS_RES_WINERR;
			::CloseHandle(pTbl->pWork->hComm);
			break;
		...
		// Success
		pTbl->nResult = CHTRS_RES_OK;
		bRet = TRUE;

		nWait = ::WaitForSingleObject(pTbl->pWork->hMutex, INFINITE);
		if (nWait != WAIT_FAILED) {
			pTbl->pWork->DrvStat &= CHTRS_ST_DTR;
			pTbl->pWork->DrvStat |= CHTRS_ST_RTS;
	...
	do{
		...
		if(pTbl->pWork->hSimObject == INVALID_HANDLE_VALUE ){
			CHTrs_ShowLastError(NULL);
			break;
		}
#ifdef _TRACE_SERVER_
	CHTrs_Trace("InitComPort - Create MailSlot %s", pTbl->pWork->ReceiveSlotName);
#endif
	...
	#ifdef _TRACE_SERVER_
			CHTrs_Trace("InitComPort - Create NamedPipe %s", name_);
	#endif
			// 连接客户端,若失败则关闭管道 
...
#ifdef _TRACE_SERVER_
	CHTrs_Trace("InitComPort - Exit - return %d", bRet);
#endif
	return bRet;
}
...
#ifdef _TRACE_SERVER_
	CHTrs_Trace("EndComPort - Exit - return %d", bRet);
#endif
	return bRet;
}
...
{
#ifdef _TRACE_SERVER_
	CHTrs_Trace("GetComunicationMethod - Enter");
#endif
	LONG	lRet;
	HKEY	hkey = NULL;
	DWORD	dwType, dwSize ;
	...
	//int CommunicationMethod = GetPrivateProfileInt(CH_CONFIG_SECTION,CH_CONMU_MECHOD, COM_PORT, szAppPath);
	...
	if(pTbl->pWork->bCommunicationMethod == MAIL_SLOT){
		...
		//do{
		//	// 打开注册表
		//	lRet = ::RegOpenKeyEx(HKEY_CURRENT_USER,"Software\\CHInfo",0,KEY_READ,&hkey);
		...
		//	}
		//	else{
		//		strcpy( (char *)pTbl->pWork->SendSlotName, "\\\\.\\mailslot\\Driver2CH");
		//	}
		//	
		//	// 取得接收邮槽名称
		//	dwSize = MAX_PATH;
		...
		//	}
		//	else{
		//		strcpy( (char *)pTbl->pWork->ReceiveSlotName, "\\\\.\\mailslot\\CH2Driver" );
		//	}
...
#ifdef _TRACE_SERVER_
	CHTrs_Trace("GetComunicationMethod - Exit - return %d", pTbl->pWork->bCommunicationMethod);
#endif
}
...
BOOL IdlCheck(IN PDEVICETBL pTbl)
{
...
#ifdef _TRACE_SERVER_
	CHTrs_Trace("IdlCheck - Has buffered data");
#endif
		// 激活接收数据事件对象
	...
	// 服务状态为空闲并且发送队列存在数据项
	else if (((pWork->DrvStat & CHTRS_ST_SNDEXE) == 0) && (pWork->pSndQue->nCnt != 0)){
#ifdef _TRACE_SERVER_
	CHTrs_Trace("IdlCheck - Has request on queue");
#endif
		// 激活发送数据事件对象
...
#ifdef _TRACE_SERVER_
	CHTrs_Trace("IdlCheck - Exit - return %d", bRet);
#endif
	return bRet;
}
...
@param      (o)CHTRS_RESULT* pRes  命令执行结果
...
*/
//////////////////////////////////////////////////////////////////////////
BOOL GetSendRequest(IN HTDLWORK* pWork,OUT SENDITEM* pItem,OUT CHTRS_RESULT* pRes)
{
#ifdef _TRACE_SERVER_
	CHTrs_Trace("GetSendRequest - Enter");
#endif
	BOOL bRet = FALSE;
	DWORD nResult;
	HANDLE hMutex = pWork->hMutex;				// Synchronize Sending/Receiving
	SEND_QUEUE* pQue = pWork->pSndQue;			// Send queue
	*pRes = CHTRS_RES_WINERR;					// Suppose operation fail
...
#ifdef _TRACE_SERVER_
	CHTrs_Trace("GetSendRequest - Exit - return %d", bRet);
#endif
	return bRet;
}
...
{
#ifdef _TRACE_SERVER_
	CHTrs_Trace("GetRecvRequest - Enter");
#endif
	BOOL bRet = FALSE;
	HANDLE hMutex = pWork->hMutex;			// Synchronize Sending/Receiving
	...
	if (nResult != WAIT_FAILED){
		...
		if(pQue->nCnt > 0){
			...
			if(::ReleaseMutex(hMutex) != 0){
				...
				*pRes = CHTRS_RES_OK;				// Set return code
			}
		}
		// 队列空
		else{
...
#ifdef _TRACE_SERVER_
	CHTrs_Trace("GetRecvRequest - Exit - return %d", bRet);
#endif
	return bRet;
}
...
{
#ifdef _TRACE_SERVER_
	CHTrs_Trace("SendEvent - Enter");
#endif
	BOOL bRet = FALSE;
	PHTDLWORK pWork = pTbl->pWork;
	CHTRS_RESULT nResult;
	DWORD nWait;
	do{
		// 检查服务状态
		if ((pWork->DrvStat & CHTRS_ST_SNDEXE) != 0){
			// 服务忙，正在发送数据项
			bRet = TRUE;
			break;
		}
		...
		// 初始化发送操作结果为失败
		*(pWork->SndItem.pResult) = CHTRS_RES_WINERR;

		// 设置服务状态忙，为正在发送数据
		UpdateStatus(pWork, CHTRS_ST_SNDEXE, TRUE);	
...
//---------------------------------------------------------------------
		...
		*pWork->SndItem.pResult =	CHTRS_HTDL_ERR_SENDFAIL;	// 初始为发送失败
...
//---------------------------------------------------------------------
		...
		UpdateStatus(pWork, CHTRS_ST_SNDEXE, FALSE);
...
#ifdef _TRACE_SERVER_
	CHTrs_Trace("SendEvent - Exit - return %d", bRet);
#endif
	return bRet;
}
...
BOOL RecvEvent(IN PDEVICETBL pTbl)
{
	...
	do{
		...
		if (pWork->pBuffQue->nCnt == 0){
			bRet = TRUE;
			*(pWork->RcvItem.pResult) = CHTRS_RECV_EMPTY;
		...
		*(pWork->RcvItem.pRecvLength) = pBufferItem->nWP;
		...
		if (pWork->pBuffQue->nCnt == 0){
			UpdateStatus(pWork, CHTRS_ST_RCVQUE, FALSE);
		}

		//
		*(pWork->RcvItem.pResult) = CHTRS_RES_OK;
		::SetEvent(pWork->RcvItem.hEvent);
		bRet = TRUE;
	} while (0) ;

#ifdef _TRACE_SERVER_
	CHTrs_Trace("RecvEvent - Exit - return %d", bRet);
#endif
	return bRet;
}
...
//#ifdef _TRACE_SERVER_
//	CHTrs_Trace("RecvData - Lost block found after block %02X", pWork->LastBlockNumRecv);
//#endif
//					break;
//				}
...
//			else{
//#ifdef _TRACE_SERVER_
//	CHTrs_Trace("RecvData - middle Block Over flow. Now len = %d, Max allow = %d", pItem->nWP, pWork->pBuffQue->nMaxBuffSize);
//#endif
//				UpdateStatus(pWork, CHTRS_ST_RCVOVR, TRUE);	//
//			}
//		}
		...
		if(TRUE == bAlert){
			...
			UpdateStatus(pWork, CHTRS_ST_RCVQUE, TRUE);	
		}
		bRet = TRUE;
	}
	while (0);
...
#ifdef _TRACE_SERVER_
	CHTrs_Trace("RecvData - Exit - return %d", bRet);
#endif
	return bRet;
}
...
#ifdef _TRACE_SERVER_
	CHTrs_Trace("OpenHTDL - Exit - return %d", bRet);
#endif
	return bRet;
}
...
#ifdef _TRACE_SERVER_
	CHTrs_Trace("CloseHTDL - Exit - return %d", bRet);
#endif
	return bRet;
}
...
#ifdef _TRACE_SERVER_
	CHTrs_Trace("InitServThread - Enter");
#endif
	_ASSERTE(NULL != pTbl);
	do{
		// 分配工作结构空间
		...
		if (pTbl->pWork == NULL){
			pTbl->nResult = CHTRS_HTDL_ERR_WORKNG;
			break;
		}
		pWork = pTbl->pWork;
		...
		if(pWork->pSndQue == NULL){
			pTbl->nResult = CHTRS_HTDL_ERR_SQUENG;
			break;
		}
...
#ifdef _TRACE_SERVER_
	CHTrs_Trace("InitServThread - Exit - return %d", bRet);
#endif
	return bRet;
}
...
{
#ifdef _TRACE_SERVER_
	CHTrs_Trace("EndServThread - Enter");
#endif
...
#ifdef _TRACE_SERVER_
	CHTrs_Trace("EndServThread - Exit");
#endif
}
...
#ifdef _TRACE_SERVER_
	CHTrs_Trace("ServThread - Exit - return 0");
#endif
	return 0;
}