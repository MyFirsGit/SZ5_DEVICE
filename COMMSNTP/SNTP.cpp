...
struct NtpBasicInfo
{
	...
	CNtpTimeStamp m_ReferenceTimestamp;
	CNtpTimeStamp m_OriginateTimestamp;
	CNtpTimeStamp m_ReceiveTimestamp;
	CNtpTimeStamp m_TransmitTimestamp;
};
...
CNtpTime::CNtpTime(CNtpTimeStamp& packet)
{
...
CNtpTime::operator CNtpTimeStamp() const
{
	CNtpTimeStamp ntp;
...
BOOL CNtpSocket::Connect(LPCTSTR pszHostAddress, int nPort)
{
	...
	// Modify by zhengxianle at 20150618
	USES_CONVERSION_EX;
	...
	LPSTR lpszAscii;
	if(pszHostAddress != NULL){
		lpszAscii = T2A_EX((LPTSTR)pszHostAddress,_ATL_SAFE_ALLOCA_DEF_THRESHOLD);
		if(NULL == lpszAscii){
			// OUT OF MEMORY
	...
	//must have been created first
//	ASSERT(m_hSocket != INVALID_SOCKET);
//	LPCSTR lpszAscii = _T2CA(pszHostAddress);
//// #ifdef UNICODE
//// 	//For correct operation of the T2A macro, see MFC Tech Note 59
//// 	USES_CONVERSION;
//// 	lpszAscii = T2A(pszHostAddress);
//// #else
//// 	lpszAscii = pszHostAddress;
//// #endif
//
...
INT CSNTPClient::GetServerTime(LPCTSTR pszHostName, NtpServerResponse& response, int nPort)
{
	//For correct operation of the T2A macro, see MFC Tech Note 59
	//USES_CONVERSION;
	...
	if (!pSocket->Connect(pszHostName, nPort)){	
		...
		if (!pSocket->Send((LPCSTR) &nbi, nSendSize)){
			SafeDelete(pSocket);
			return NTP_SEND_REQUEST_FAILURE;
		}

		//Need to use select to determine readabilty of socket
		...
		int iRevRet = pSocket->Receive((LPSTR) &nfp, nReceiveSize);
		if(iRevRet <=0){
			SafeDelete(pSocket);
			return NTP_READ_APLY_FAILURE;
		}
...
BOOL CSNTPClient::SetClientTime(const CNtpTime& NewTime)
{
	...
	if (EnableSetTimePriviledge())
	{
		...
		if(st.wYear<1955)//如果小于1955年就不同步了
		{
			return bSuccess;
		}
