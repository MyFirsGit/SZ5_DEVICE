...
{
	unsigned long ret = 0;
	m_pipeStat = ST_SNDEXE;
	// 连接异常
	if(!pipe_){
		m_pipeStat = ST_SNDERR;
		ret = 0;
	}
	// 连接正常
	else{
		...
		m_pipeStat = (ret > 0)?ST_RCVQUE:ST_SNDERR;
	}
	return ret;
}
...
{
	unsigned long ret = 0;
	m_pipeStat = ST_RCVEXE;
	// 连接异常
	if(!pipe_){
		m_pipeStat = ST_RCVERR;
		ret = 0;
	}
	// 连接正常
	else{
		...
		m_pipeStat = (ret > 0)?ST_SNDQUE:ST_RCVERR;
	}
	return ret;
}
