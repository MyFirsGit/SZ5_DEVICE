...
BOOL GetSendRequest(IN HTDLWORK* pWork, IN SENDITEM* pItem, OUT COMM_RESULT* pRes);
BOOL GetRecvRequest(IN HTDLWORK* pWork,	IN RECVITEM* pItem,	OUT COMM_RESULT* pRes);
...
BOOL InitComPort(IN PDEVICETBL pTbl)
{
	...
	do{
		...
		if (pTbl->Parm.ComPort<10)	{
			sprintf_s(buff,128 ,"COM%d", (pTbl->Parm.ComPort));	
		}
		else{
			sprintf_s(buff, 128,"\\\\.\\COM%d", (pTbl->Parm.ComPort));
		}
		...
		if (nWait != WAIT_FAILED) {
			pTbl->pWork->DrvStat &= ST_DTR;
			pTbl->pWork->DrvStat |= ST_RTS;
...
BOOL IdlCheck(IN PDEVICETBL pTbl)
{
	...
	// 服务状态为空闲并且发送队列存在数据项
	else if (((pWork->DrvStat & ST_SNDEXE) == 0) && (pWork->pSndQue->nCnt != 0)){
#ifdef _TRACE_SERVER_
...
@param      (o)COMM_RESULT* pRes  命令执行结果
...
*/
//////////////////////////////////////////////////////////////////////////
BOOL GetSendRequest(IN HTDLWORK* pWork,OUT SENDITEM* pItem,OUT COMM_RESULT* pRes)
{
#ifdef _TRACE_SERVER_
	...
	*pRes = ERR_WINERR;					// Suppose operation fail
...
BOOL SendEvent(IN PDEVICETBL pTbl)
{
	...
	do{
		...
		// 初始化发送操作结果为失败
		*(pWork->SndItem.pResult) = ERR_WINERR;

		// 设置服务状态忙，为正在发送数据
		UpdateStatus(pWork, ST_SNDEXE, TRUE);	
...
//---------------------------------------------------------------------
		...
		*pWork->SndItem.pResult =	HTDL_ERR_SENDFAIL;	// 初始为发送失败
...
//---------------------------------------------------------------------
		...
		UpdateStatus(pWork, ST_SNDEXE, FALSE);
...
BOOL RecvEvent(IN PDEVICETBL pTbl)
{
	...
	do{
		...
		if (pWork->pBuffQue->nCnt == 0){
			bRet = TRUE;
			*(pWork->RcvItem.pResult) = ERR_RECV_EMPTY;
		...
		if (pWork->RcvItem.pBuffer[14] != 0xD0 && pWork->RcvItem.pBuffer[14] != 0x65) {
			...
			for(int i=0;i < logLength; i++) {
				sprintf_s(&log[2*i+2],3,"%02x",pWork->RcvItem.pBuffer[i]);
			}
			// 打印日志
			CTSPTrs_Trace(log);
		}
		...
		*(pWork->RcvItem.pRecvLength) = pBufferItem->nWP;
		...
		if (pWork->pBuffQue->nCnt == 0){
			UpdateStatus(pWork, ST_RCVQUE, FALSE);
		}

		//
		*(pWork->RcvItem.pResult) = RES_OK;
		::SetEvent(pWork->RcvItem.hEvent);
		bRet = TRUE;
	} while (0) ;
...
BOOL RecvData(IN HTDLWORK *pWork,IN BYTE *data,IN int cnt)
{
	...
	do {
		...
		if(TRUE == bAlert){
			...
			UpdateStatus(pWork, ST_RCVQUE, TRUE);	
		}
		bRet = TRUE;
	}
	while (0);
