...
DWORD WINAPI HtdlThread(LPVOID pHTDLWork)
{
	...
	::SetEvent(pWork->hSync);
#ifdef _TRACE_HTDL_
	IOTrs_Trace("HtdlThread - Exit - return %d", 0);
#endif
	return 0;
}
...
int CheckEvent(HTDLWORK *pWork)
{
	...
	for (;;) {
		...
		if(chk == FALSE){
#ifdef _TRACE_HTDL_
	IOTrs_Trace("CheckEvent - IOTRS_ST_RCVERR");
#endif
			// 设置状态为接收失败
			pWork->DrvStat |= IOTRS_ST_RCVERR;	
		}
		// 成功
		else{
			if(nBytesRead > 0){
		...
		if(chk == TRUE){
#ifdef _TRACE_HTDL_
	IOTrs_Trace("CheckEvent - EVENT_TIMEOUT");
#endif
...
#ifdef _TRACE_HTDL_
	IOTrs_Trace("CheckEvent - EVENT_REQSEND");
#endif
...
#ifdef _TRACE_HTDL_
	IOTrs_Trace("CheckEvent - Exit - return %d", iEvent);
#endif
	pWork->Event = iEvent;
...
#ifdef _TRACE_HTDL_
	IOTrs_Trace("PreEvent - Exit - return %d", iEvent);
#endif
	return iEvent;
}
...
#ifdef _TRACE_HTDL_
	IOTrs_Trace("ChkEventChr - Exit - return %d", ret);
#endif
	return ret;
}
...
#ifdef _TRACE_HTDL_
	IOTrs_Trace("IsTimeUp - Exit - return %d", bRet);
#endif
	return bRet;
}
...
{
#ifdef _TRACE_HTDL_
	IOTrs_Trace("TimerSetForPhase - Enter");
#endif
...
#ifdef _TRACE_HTDL_
	IOTrs_Trace("TimerSetForPhase - Exit");
#endif
}