...
{
	unsigned long ret = 0;
	m_pipeStat = IOTRS_ST_SNDEXE;
	// 连接异常
	if(!pipe_){
		m_pipeStat = IOTRS_ST_SNDERR;
		ret = 0;
	}
	// 连接正常
	else{
		...
		m_pipeStat = (ret > 0)?IOTRS_ST_RCVQUE:IOTRS_ST_SNDERR;
	}
	return ret;
}
...
{
	unsigned long ret = 0;
	m_pipeStat = IOTRS_ST_RCVEXE;
	// 连接异常
	if(!pipe_){
		m_pipeStat = IOTRS_ST_RCVERR;
		ret = 0;
	}
	// 连接正常
	else{
		...
		m_pipeStat = (ret > 0)?IOTRS_ST_SNDQUE:IOTRS_ST_RCVERR;
	}
	return ret;
}
