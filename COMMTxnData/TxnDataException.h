...
class TXNDATA_API CTxnDataException : public CSysException
{
public:
    static const int MODULE_ID = 10106;			// 模块号
    ...
    typedef enum _detail_error_code {
		...
		NOT_SEND_FILE_ERR			= 510,	    // 数据文件不是未送文件

		DATA_NOT_SAME_ERR 			= 601,	    // 设定已送标志时传入数据与保存数据不一致
        
		SALEDATA_NOT_SEND_TOO_MORE	= 701,		// 未送数据过多

		VALIDATE_NOT_SEND_FAILD     = 702,		//校验未送数据失败

		MALLOC_FAIL					= 801,		// 分配内存失败
    } DETAIL_ERROR_CODE;

    CTxnDataException(DETAIL_ERROR_CODE lDetailID, CString sFileName, long lLineNo, CString sMessage = _T(""));
    ~CTxnDataException(void);
