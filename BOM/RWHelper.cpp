...
@param       (i)bool reset      是否reset   true:执行 RWReset, false:不执行 RWReset
...
void CRWHelper::InitData()
{
	...
	while(!hasInit && iResult == RW_RSP_OK){
		switch(Status){
			...
			default:
				{
					throw CRWException(CRWException::FUNC_ERR_RETURN,_T(__FILE__),__LINE__,_T("TPU返回状态值非法"));
				}
		}
	}
}
...
bool CRWHelper::UpdateParamter(CParamHelper::VEC_PARAM_ID vecParamId)
{
	...
	while (iteId != vecParamId.end()) {
		...
		if(*iteId == AFC_MODEHISTORY_ID || *iteId == AFC_DEVICECOMMONPARAM_ID || *iteId == AFC_STAFFPWDPARAM_ID
			...
			throw CRWException(CRWException::FUNC_PARAM_ERR,_T(__FILE__),__LINE__,_T("读写器返回的参数ID异常:" + sParaID));
		}
		...
		if(thePARAM_HELPER.GetFileNameFromFullPath(paramPathName).GetLength()!=0){
			...
			CString  sParaID = _T("");
			sParaID.Format(_T("%.4x"),*iteId);  
			throw CRWException(CRWException::FUNC_PARAM_ERR,_T(__FILE__),__LINE__,_T("读写器同步用本地参数不存在:" + sParaID));
		}
		iteId ++;
		//Sleep(1000);
	}
			...
			for(int i = 0;i<paramCnt;i++){
				...
				switch(paraID){
					...
					default:
						...
						//throw CRWException(errCode, _T(__FILE__), __LINE__,_T("外部读写器返回的参数ID异常:" + sParaID));						
				}
			}
		}
	}
