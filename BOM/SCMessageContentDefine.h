...
struct ParameterAndSoftwareUpdate
{
	...
	ParameterAndSoftwareUpdate(LPBYTE lpData)
	{
		...
		parameterID = (PARAMETER_ID)mstos(sParameterID);
		lpData += 2;
		short sProviderID;
		...
		lpData +=4;
		//downLoadPath.SetString((LPSTR)lpData,64);
	...
	{
		TCHAR emptyDownloadPath[64] = {0x00};
		downLoadPath.SetString(emptyDownloadPath);
	}
	...
	void Serialize(LPBYTE lpData)
	{
		...
		lpData +=4;
		memcpy(lpData, downLoadPath.GetString(),64);
		lpData +=64;
	...
	bool SaveToFile(CString& fileName)
	{
		BYTE data[84];
		Serialize(data);
		CXFile cFile(fileName,_T(""));
		cFile.WriteFileWithoutSUM((char*)data,84);
