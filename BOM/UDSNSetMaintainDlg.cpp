...
static const TCHAR* const TXT_ACC_UDSN						 =	_T("ACC流水号：");
static const TCHAR* const TXT_AFC_UDSN						 = 	_T("AFC流水号：");
static const TCHAR* const TXT_ECT_UDSN						 = 	_T("ECT流水号：");
...
{
	m_baseInfo->titleInfo.titleName = TITLE_UDSN_INFO;					   // 标题
	m_baseInfo->detailInfo.row = 3;                                        // 行数
	m_baseInfo->detailInfo.labelGroup = m_labelUDSNinfo;                   // 标签文字
	m_baseInfo->detailInfo.editGroup.cnt = 3;                              // 编辑框个数
...
void CUDSNSetMaintainDlg::InitLabelUDSNInfo()
{
	...
	{
		// 初始化USDN信息区域文字
		m_labelUDSNinfo[0].nameLeft = add_wsp(TXT_ACC_UDSN, MAX_SBC_CASE_LEN);
		m_labelUDSNinfo[1].nameLeft = add_wsp(TXT_AFC_UDSN, MAX_SBC_CASE_LEN);
		m_labelUDSNinfo[2].nameLeft = add_wsp(TXT_ECT_UDSN, MAX_SBC_CASE_LEN);
	}
	catch(CSysException&)
	{
		throw;
...
void CUDSNSetMaintainDlg::SetUDSNInfoToSvc()
{
	...
	try
	{
		...
		pUDSNSetSvc->SetEctUDSN(_tcstoul(GetEditText(UDSNINFO_EDIT_GROUP_ECTUDSN), NULL, 10));
...
LRESULT  CUDSNSetMaintainDlg::OnOK(WPARAM wParam,LPARAM lParam)
{
	...
	{		
		...
		if (CheckInputText())
		{
			...
			SetUDSNInfoToSvc();
			//
			m_labelUDSNinfo[0].nameRight = add_hsp_right(GetEditText(UDSNINFO_EDIT_GROUP_ACCUDSN), MAX_SBC_CASE_LEN);
			m_labelUDSNinfo[1].nameRight = add_hsp_right(GetEditText(UDSNINFO_EDIT_GROUP_AFCUDSN), MAX_SBC_CASE_LEN);
			m_labelUDSNinfo[2].nameRight = add_hsp_right(GetEditText(UDSNINFO_EDIT_GROUP_ECTUDSN), MAX_SBC_CASE_LEN);
			// 显示设置成功
