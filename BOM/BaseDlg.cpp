...
int CBaseDlg::OnCreate(LPCREATESTRUCT lpCreateStruct)
{
    try {
        ...
        //theEXCEPTION_MGR.ProcessException(CInnerException(GetService()->GetServiceID(), CInnerException::OTHER_ERR, __FILE__, __LINE__));
    }
...
BOOL CBaseDlg::PreTranslateMessage(MSG* pMsg)
{
		...
		)
	{
		...
		{
			CXButton* button = dynamic_cast<CXButton*>(current);
			button->SetFocus();
...
{
    int height = -MulDiv(theBOM_INFO.GetOperateBtnFontSize(), GetDeviceCaps(*pDC, LOGPIXELSY), 72);

    CFont font;
    font.CreateFont(height, 0, 0, 0, FW_HEAVY, FALSE, FALSE, 0,
        GB2312_CHARSET, OUT_DEFAULT_PRECIS, CLIP_DEFAULT_PRECIS, PROOF_QUALITY,
        DEFAULT_PITCH | FF_DONTCARE, theBOM_INFO.GetOperateBtnFont());
