...
BOOL CGEdit::Create()
{
	DWORD dwStyle = WS_CHILD | WS_VISIBLE  | ES_UPPERCASE|WS_TABSTOP;
...
bool CGEdit::IsValidChar(UINT nChar)
{
	...
	case CASH:
		{
			...
			if(dotPos==-1)//没有小数点
			{
				if(isdigit(nChar) && (GetLimitText() -3)<=strText.GetLength())
				{
					return false;
				}
				if (isdigit(nChar)||nChar == '.') {
	...
	case NAME:
		{
			if (!isdigit(nChar)&&!ispunct(nChar)&&!iscntrl(nChar)) {
					return true;
			}			
			return false;
		}
