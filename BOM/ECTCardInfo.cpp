	...
	try {
		...
		if (TicketPhyType == MIFARE1K){
			for(int i=0; i<count; i++){
				...
				case LOACLPAK_CONSUMER:
					history.TransType = _T("消费");
					break;
				case UPDATE_TRAC:
					history.TransType = _T("补票");
					break;
				case REFOUND_TRAC:
					history.TransType = _T("退卡");
					break;
				case LUCK_TRAC:
					history.TransType = _T("锁卡");
					break;
				case ENTRE_TRAC:
					history.TransType = _T("进站");
					break;
				case OUT_TRAC:
					history.TransType = _T("出站");
					break;
				case CHARGE_TRAC:
					history.TransType = _T("充值");
					break;
				default :
					history.TransType ="--"; 
					break;
		...
		//一卡通CPU卡
			...
			for(int i=0; i<count; i++){	
				...
				switch (type){
				case MONTHPAK_STROE:
					history.TransType = _T("月票钱包 圈存");
					break;
				case PUBPAK_STROE:
					history.TransType = _T("公共钱包 圈存");
					break;
				case MONTHPAK_CONSUMER:
					history.TransType = _T("月票钱包 消费");
					break;
				case PUBPAK_CONSUMER:
					history.TransType = _T("公共钱包 消费");
					break;
				case COM_CONSUMER:
					history.TransType = _T("复合消费");
					break;
				default :
					history.TransType =_T("--"); 
					break;
				}
