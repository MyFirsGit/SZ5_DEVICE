...
void CParamMsg::SetAfcParameterVersion()
{
	...
	//// BOM运营参数类
	//m_version_afc.lBomOperationParam = theAFC_BOM_OPERATION.GetCurMasterVer();
	//// 设备公共参数类
...
void CParamMsg::SaveMsg(LPBYTE lpData)
{
	...
	try {
		...
		// 删除旧版本参数文件
		CString sOldFileName = thePARAM_HELPER.GetTheParamPathName(0x1002,PARAM_AFC_CURRENT_DIR);
		DeleteFile(sOldFileName);

		CString sOldBackUpFileName = thePARAM_HELPER.GetTheParamBackUpPathName(0x1002,PARAM_AFC_CURRENT_DIR);
		...
		CString csFileName = theBOM_INFO.GetDataPath() + PARAM_AFC_CURRENT_DIR + AFC_PARAM_ID_1002 + stationCode + _T(".") + csVersion +_T(".00");
		CString csFileNameBackup = theBOM_INFO.GetBackupPath()+ PARAM_AFC_CURRENT_DIR + AFC_PARAM_ID_1002 + stationCode + _T(".") + csVersion +_T(".00");

		auto_ptr<CXFile> FileMgr(new CXFile(csFileName,csFileNameBackup));
		FileMgr->WriteFileWithoutSUM(pData, pDataSize);
		FileMgr->CopyXFile();
