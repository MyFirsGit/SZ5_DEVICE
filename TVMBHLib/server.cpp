...
BOOL GetSendRequest(IN HTDLWORK* pWork, IN SENDITEM* pItem, OUT BHTRS_RESULT* pRes);
BOOL GetRecvRequest(IN HTDLWORK* pWork,	IN RECVITEM* pItem,	OUT BHTRS_RESULT* pRes);
...
BOOL InitComPort(IN PDEVICETBL pTbl)
{
	...
	DWORD nWait;
		...
		BHTrs_Trace("InitComPort - Enter");
	#endif

	pTbl->nResult = BHTRS_RES_WINERR;
	// 串口通信
	do{
		...
		if (pTbl->pWork->hComm == INVALID_HANDLE_VALUE){
			pTbl->nResult = BHTRS_HTDL_ERR_PORT;
			break;
		}
		// 设置输入输出缓存大小
		...
		if (FALSE == ::GetCommState(pTbl->pWork->hComm, &dcb)){
			#ifdef _TRACE_SERVER_
				BHTrs_Trace("InitComPort - GetCommState() = FALSE");
			#endif
			pTbl->nResult = BHTRS_RES_WINERR;
		...
		dcb.fParity = TRUE;					    // 允许奇偶校验(偶校验：捷德)
		dcb.BaudRate = pTbl->Parm.Speed;		// 设置波特率
		dcb.StopBits = ONESTOPBIT;				// 停止位(1)
		dcb.Parity = ODDPARITY/*NOPARITY*/;	    // 奇偶校验方法(偶校验：捷德)
		dcb.ByteSize = 8;                       // 数据位
		...
		if (FALSE == ::SetCommState(pTbl->pWork->hComm, &dcb)){
			#ifdef _TRACE_SERVER_
				BHTrs_Trace("InitComPort - SetCommState() = FALSE");
			#endif
			pTbl->nResult = BHTRS_RES_WINERR;
			::CloseHandle(pTbl->pWork->hComm);
			break;
		...
		if (FALSE == ::SetCommTimeouts(pTbl->pWork->hComm, &timeout)){
			#ifdef _TRACE_SERVER_
				BHTrs_Trace("InitComPort - SetCommTimeouts() = FALSE");
			#endif
			pTbl->nResult = BHTRS_RES_WINERR;
			::CloseHandle(pTbl->pWork->hComm);
			break;
		...
		// Success
		pTbl->nResult = BHTRS_RES_OK;
		bRet = TRUE;

		nWait = ::WaitForSingleObject(pTbl->pWork->hMutex, INFINITE);
		if (nWait != WAIT_FAILED) {
			pTbl->pWork->DrvStat &= BHTRS_ST_DTR;
			pTbl->pWork->DrvStat |= BHTRS_ST_RTS;
	...
	do{
		...
		if(pTbl->pWork->hSimObject == INVALID_HANDLE_VALUE ){
			BHTrs_ShowLastError(NULL);
			break;
		}
#ifdef _TRACE_SERVER_
	BHTrs_Trace("InitComPort - Create MailSlot %s", pTbl->pWork->ReceiveSlotName);
#endif
	...
	#ifdef _TRACE_SERVER_
			BHTrs_Trace("InitComPort - Create NamedPipe %s", name_);
	#endif
			// 连接客户端,若失败则关闭管道 
...
#ifdef _TRACE_SERVER_
	BHTrs_Trace("InitComPort - Exit - return %d", bRet);
#endif
	return bRet;
}
...
#ifdef _TRACE_SERVER_
	BHTrs_Trace("EndComPort - Exit - return %d", bRet);
#endif
	return bRet;
}
...
{
#ifdef _TRACE_SERVER_
	BHTrs_Trace("GetComunicationMethod - Enter");
#endif
	LONG	lRet;
	HKEY	hkey = NULL;
	DWORD	dwType, dwSize ;
	char	sBuf[128], sData[MAX_PATH];

	// 取得通信配置
	//TCHAR szAppPath[MAX_PATH];
	//ComGetAppPath(szAppPath,MAX_PATH);
	//strcat(szAppPath,DEVICE_INI_FILE_NAME);
	//int CommunicationMethod = GetPrivateProfileInt(BH_CONFIG_SECTION,BH_CONMU_MEBHOD, COM_PORT, szAppPath);
	...
	if(pTbl->pWork->bCommunicationMethod == MAIL_SLOT){
		...
		//do{
		//	// 打开注册表
		//	lRet = ::RegOpenKeyEx(HKEY_CURRENT_USER,"Software\\BHInfo",0,KEY_READ,&hkey);
		...
		//	}
		//	else{
		//		strcpy( (char *)pTbl->pWork->SendSlotName, "\\\\.\\mailslot\\Driver2BH");
		//	}
		//	
		//	// 取得接收邮槽名称
		//	dwSize = MAX_PATH;
		...
		//	}
		//	else{
		//		strcpy( (char *)pTbl->pWork->ReceiveSlotName, "\\\\.\\mailslot\\BH2Driver" );
		//	}
...
#ifdef _TRACE_SERVER_
	BHTrs_Trace("GetComunicationMethod - Exit - return %d", pTbl->pWork->bCommunicationMethod);
#endif
}
...
BOOL IdlCheck(IN PDEVICETBL pTbl)
{
...
#ifdef _TRACE_SERVER_
	BHTrs_Trace("IdlCheck - Has buffered data");
#endif
		// 激活接收数据事件对象
	...
	// 服务状态为空闲并且发送队列存在数据项,且当前不为空闲状态
	else if (((pWork->DrvStat & BHTRS_ST_SNDEXE) == 0) && (pWork->pSndQue->nCnt > 0) && pWork->Phase == PHASE_S1 && pWork->LastEvent != EVENT_OFF){
#ifdef _TRACE_SERVER_
	BHTrs_Trace("IdlCheck - Has request on queue");
#endif
		// 激活发送数据事件对象
		if(::SetEvent(pTbl->hSend) == FALSE){
			bRet = FALSE;
		}
		BHTrs_Trace("Send item!");
	}

#ifdef _TRACE_SERVER_
	BHTrs_Trace("IdlCheck - Exit - return %d", bRet);
#endif
	return bRet;
}
...
@param      (o)BHTRS_RESULT* pRes  命令执行结果
...
*/
//////////////////////////////////////////////////////////////////////////
BOOL GetSendRequest(IN HTDLWORK* pWork,OUT SENDITEM* pItem,OUT BHTRS_RESULT* pRes)
{
#ifdef _TRACE_SERVER_
	BHTrs_Trace("GetSendRequest - Enter");
#endif
	BOOL bRet = FALSE;
	DWORD nResult;
	HANDLE hMutex = pWork->hMutex;				// Synchronize Sending/Receiving
	SEND_QUEUE* pQue = pWork->pSndQue;			// Send queue
	*pRes = BHTRS_RES_WINERR;					// Suppose operation fail
...
#ifdef _TRACE_SERVER_
	BHTrs_Trace("GetSendRequest - Exit - return %d", bRet);
#endif
	return bRet;
}
...
{
#ifdef _TRACE_SERVER_
	BHTrs_Trace("GetRecvRequest - Enter");
#endif
	BOOL bRet = FALSE;
	HANDLE hMutex = pWork->hMutex;			// Synchronize Sending/Receiving
	...
	if (nResult != WAIT_FAILED){
		...
		if(pQue->nCnt > 0){
			...
			if(::ReleaseMutex(hMutex) != 0){
				...
				*pRes = BHTRS_RES_OK;				// Set return code
			}
		}
		// 队列空
		else{
...
#ifdef _TRACE_SERVER_
	BHTrs_Trace("GetRecvRequest - Exit - return %d", bRet);
#endif
	return bRet;
}
...
{
#ifdef _TRACE_SERVER_
	BHTrs_Trace("SendEvent - Enter");
#endif
	BOOL bRet = FALSE;
	PHTDLWORK pWork = pTbl->pWork;
	BHTRS_RESULT nResult;
	DWORD nWait;
	do{
		// 检查服务状态
		if ((pWork->DrvStat & BHTRS_ST_SNDEXE) != 0 || (pWork->DrvStat & BHTRS_ST_RCVQUE) != 0 || pWork->Phase != PHASE_S1){
			// 服务忙，正在收发送数据项
			bRet = TRUE;
			break;
		}
		...
		// 初始化发送操作结果为失败
		*(pWork->SndItem.pResult) = BHTRS_RES_WINERR;

		// 设置服务状态忙，为正在发送数据
		UpdateStatus(pWork, BHTRS_ST_SNDEXE, TRUE);	
...
//---------------------------------------------------------------------
		...
		*pWork->SndItem.pResult =	BHTRS_HTDL_ERR_SENDFAIL;	// 初始为发送失败
...
//---------------------------------------------------------------------
		...
		UpdateStatus(pWork, BHTRS_ST_SNDEXE, FALSE);
	...
	// 激活数据项发送完成事件（确认当前已发送完成，防止下一个命令过来上一个未执行完导致异常）
	/*if ((pWork->DrvStat & BHTRS_ST_SNDEXE) == 0){
	::SetEvent(pWork->SndItem.hEvent);
	}*/
	

#ifdef _TRACE_SERVER_
	BHTrs_Trace("SendEvent - Exit - return %d", bRet);
#endif
	return bRet;
}
...
BOOL RecvEvent(IN PDEVICETBL pTbl)
{
	...
	do{
		...
		if (pWork->pBuffQue->nCnt == 0){
			bRet = TRUE;
			*(pWork->RcvItem.pResult) = BHTRS_RECV_EMPTY;
		...
		if (pWork->RcvItem.pBuffer[14] != 0xD0 && pWork->RcvItem.pBuffer[14] != 0x65) {
			...
			/*for(int i=0;i < logLength; i++) {
				sprintf(&log[i+2],"%c",pWork->RcvItem.pBuffer[i]);
			}*/
			memcpy(&log[2],pWork->RcvItem.pBuffer,logLength);
			BHTrs_Trace(log);
		}
		...
		*(pWork->RcvItem.pRecvLength) = pBufferItem->nWP;
		...
		//
		*(pWork->RcvItem.pResult) = BHTRS_RES_OK;
		::SetEvent(pWork->RcvItem.hEvent);
		bRet = TRUE;
	} while (0) ;

#ifdef _TRACE_SERVER_
	BHTrs_Trace("RecvEvent - Exit - return %d", bRet);
#endif
	return bRet;
}
	...
	do {
		...
		//if (pWork->BlockNumRecv == 0x00 || pWork->BlockNumRecv == 0x80){
			...
			if (pWork->pBuffQue->nCnt < pWork->pBuffQue->nMax){
				...
				pItem->nWP = cnt;
				//Keep block number for checking the next block
				//pWork->LastBlockNumRecv = pWork->BlockNumRecv;
				//Change the storing slot in the queue
				// 数据不分块
				//if(pWork->BlockNumRecv == 0x00){
					pWork->pBuffQue->nWP++;
...
//		else{
//			BUFFITEM* pItem = &pWork->pBuffQue->Que[pWork->pBuffQue->nWP];
//			if (pItem->nWP + cnt <= pWork->pBuffQue->nMaxBuffSize){
//				//Checking if lost a block
//				BYTE biCurBlock = pWork->BlockNumRecv & 0x7F; //Get 7 low bits
//				BYTE biLastBlock = pWork->LastBlockNumRecv & 0x7F; //Get 7 low bits
//				if (biLastBlock + 1 != biCurBlock){
//#ifdef _TRACE_SERVER_
//	BHTrs_Trace("RecvData - Lost block found after block %02X", pWork->LastBlockNumRecv);
//#endif
//					break;
//				}
//				::CopyMemory((pItem->pData + pItem->nWP), data, cnt);
//				pItem->nWP += cnt;
//				//Keep block number for checking the next block
//				pWork->LastBlockNumRecv = pWork->BlockNumRecv;
//				//Change the storing slot in the queue
...
//			else{
//#ifdef _TRACE_SERVER_
//	BHTrs_Trace("RecvData - middle Block Over flow. Now len = %d, Max allow = %d", pItem->nWP, pWork->pBuffQue->nMaxBuffSize);
//#endif
//				UpdateStatus(pWork, BHTRS_ST_RCVOVR, TRUE);	//
//			}
//		}
		...
		if(TRUE == bAlert){
			...
			UpdateStatus(pWork, BHTRS_ST_RCVQUE, TRUE);	
		}
		bRet = TRUE;
	}
	while (0);
...
#ifdef _TRACE_SERVER_
	BHTrs_Trace("RecvData - Exit - return %d", bRet);
#endif
	return bRet;
}
...
#ifdef _TRACE_SERVER_
	BHTrs_Trace("OpenHTDL - Exit - return %d", bRet);
#endif
	return bRet;
}
...
#ifdef _TRACE_SERVER_
	BHTrs_Trace("CloseHTDL - Exit - return %d", bRet);
#endif
	return bRet;
}
...
#ifdef _TRACE_SERVER_
	BHTrs_Trace("InitServThread - Enter");
#endif
	_ASSERTE(NULL != pTbl);
	do{
		// 分配工作结构空间
		...
		if (pTbl->pWork == NULL){
			pTbl->nResult = BHTRS_HTDL_ERR_WORKNG;
			break;
		}
		pWork = pTbl->pWork;
		...
		if(pWork->pSndQue == NULL){
			pTbl->nResult = BHTRS_HTDL_ERR_SQUENG;
			break;
		}
...
#ifdef _TRACE_SERVER_
	BHTrs_Trace("InitServThread - Exit - return %d", bRet);
#endif
	return bRet;
}
...
{
#ifdef _TRACE_SERVER_
	BHTrs_Trace("EndServThread - Enter");
#endif
...
#ifdef _TRACE_SERVER_
	BHTrs_Trace("EndServThread - Exit");
#endif
}
...
{
#ifdef _TRACE_SERVER_
	BHTrs_Trace("ServThread - Enter");
#endif
	PDEVICETBL pTbl;
	HANDLE	hEvents[3];
	DWORD nEvent;
	bool bLoop,bCheckIdl;
...
#ifdef _TRACE_SERVER_
	BHTrs_Trace("ServThread - Exit - return 0");
#endif
	return 0;
}