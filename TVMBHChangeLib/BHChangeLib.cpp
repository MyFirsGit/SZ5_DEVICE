...
// Prototype
void BHTrs_Trace(const char* lpszFormat, ...);
BHTRS_RESULT OpenCheck(IN int Handle);
BHTRS_RESULT PostSendRequest(IN BHTRS_HANDLE Handle, IN SENDITEM* pItem);
BHTRS_RESULT WaitSendComplete(IN HANDLE hEvent);
BHTRS_RESULT PostRecvRequest(IN BHTRS_HANDLE Handle, IN RECVITEM* pItem);
BHTRS_RESULT WaitRecvComplete(IN HANDLE hEvent);
INT  GetAppPath(IN OUT LPTSTR lpszBuf, IN DWORD ucchMax);
//const char* BH_TRACE_PABH_AND_NAME = "ThTrace_*.log";
...
#define BH_CONFIG_SECTION	_T("BH")			// SECTION值
#define BH_CONMU_MEBHOD		_T("CMU_MEBHOD") 	// 通信方式KEY值
...
BOOL APIENTRY DllMain(HANDLE hModule,DWORD ul_reason_for_call,LPVOID lpReserved)
{
	...
	case DLL_PROCESS_ATTACH:
		...
		iCommunicationType = GetPrivateProfileInt(BH_CONFIG_SECTION,BH_CONMU_MEBHOD, COM_PORT, szAppPath);
...
@param      (o)BHTRS_HANDLE* pHandle   已打开的连接实例句柄
@param      (i)void* pParam             指向结构体(BHTRS_CONNECT_PARAM)的指针

@retval     BHTRS_RESULT   BHTRS_RES_OK:成功   其他:失败
...
DLLIMP BHTRS_RESULT BHTrs_Open(OUT BHTRS_HANDLE* pHandle,IN  void* pParam)
{
	if(iCommunicationType == COM_PORT){
		BHTRS_RESULT nResult = BHTRS_RES_WINERR;
		DWORD dchk;
		DWORD dwTime;
		int i;
		...
		for (i = 0; i < MAX_DEVICES; i++){	
			...
			if (g_Devs[i].bUsed == false){			
				...
				nResult = BHTRS_RES_OK;
				break;
			}
		}
		// 连接池已满
		if (i >= MAX_DEVICES){
			nResult = BHTRS_HTDL_ERR_FULL;
		}
		// 离开连接池操作临界区
		if(::ReleaseMutex(g_hMutex) == FALSE){
			g_Devs[i].bUsed = false;
			nResult = BHTRS_RES_WINERR;
			return nResult;
		}

		// 连接失败则返回
		if (nResult != BHTRS_RES_OK){
			return nResult;
		}
		...
		do {
			// 保存连接参数信息
			CopyMemory(&g_Devs[i].Parm, pParam, sizeof(BHCHANGETRS_CONNECT_PARAM));
...
@param      (i)BHTRS_HANDLE Handle   要关闭连接实例句柄
...
*/
//////////////////////////////////////////////////////////////////////////
DLLIMP BHTRS_RESULT BHTrs_Close(IN BHTRS_HANDLE Handle)
{
	if(iCommunicationType == COM_PORT){
		BHTRS_RESULT nResult = BHTRS_RES_OK;
		DWORD dchk;
		int i;
...
@param      (i)BHTRS_HANDLE Handle		连接实例句柄
...
DLLIMP BHTRS_RESULT BHTrs_Send(IN BHTRS_HANDLE Handle,IN DWORD nSendLength,IN void* pBuffer,OUT DWORD* pOutLength)
{
	...
	//BHTrs_Trace("BHTrs_Send - exit - return = %08X", nResult);
	return nResult;
}
...
DLLIMP BHTRS_RESULT BHTrs_Recv(IN BHTRS_HANDLE Handle,IN DWORD nBuffLength,OUT void* pBuffer,OUT DWORD* pRecvLength)
{
		...
		*pRecvLength = 0;
		...
		if (nResult == BHTRS_RES_OK){
			...
			if (hEvent != NULL)	{
				...
				if (nRes == BHTRS_RES_OK)				//Request posted
				{
	...
	//BHTrs_Trace("BHTrs_Recv - exit - return = %08X", nResult);
	return nResult;
}
...
DLLIMP BHTRS_RESULT BHTrs_Reset(IN BHTRS_HANDLE Handle,IN BOOL bSendClear,IN BOOL bRecvClear)
{
	if(iCommunicationType == COM_PORT){
		...
		BHTRS_RESULT nResult = OpenCheck(Handle);
		do{
			// 连接有效
			if (BHTRS_RES_OK != nResult){
				break;
			}
			...
			if (TRUE == bSendClear){
				__try{
					...
					if (INVALID_HANDLE_VALUE != pWork->SndItem.hEvent){
						...
						*(pWork->SndItem.pResult) = BHTRS_RES_RESET;
			...
			if (TRUE == bRecvClear){
				__try{
					...
					if (INVALID_HANDLE_VALUE != pWork->RcvItem.hEvent){
						pWork->Phase = PHASE_S1;
						*(pWork->RcvItem.pResult) = BHTRS_RES_RESET;
		...
		*pStatus = 0;
		...
		if(nResult == BHTRS_RES_OK){
			...
			if(nWait != WAIT_FAILED){
				...
				if((Stat & MS_CTS_ON) != 0){
					g_Devs[Handle-1].pWork->DrvStat |= BHTRS_ST_CTS;
				}
				else{
					g_Devs[Handle-1].pWork->DrvStat &= ~BHTRS_ST_CTS;
				}
				if((Stat & MS_DSR_ON) != 0) {
					g_Devs[Handle-1].pWork->DrvStat |= BHTRS_ST_DSR;
				}
				else{
					g_Devs[Handle-1].pWork->DrvStat &= ~BHTRS_ST_DSR;
				}
...
@retval     BHTRS_RESULT  BHTRS_ERR_HANDLE:无效   BHTRS_RES_OK:有效
...
BHTRS_RESULT OpenCheck(IN int Handle)
{
	...
	//	BHTrs_Trace("OpenCheck - exit - Res = %08X ", nRes);
	return nRes;
}
...
@retval     BHTRS_RESULT  BHTRS_HTDL_ERR_SQUE:发送队列满   BHTRS_RES_OK:成功
...
BHTRS_RESULT PostSendRequest(IN BHTRS_HANDLE Handle,IN SENDITEM* pItem)
{
	BHTRS_RESULT nRes = BHTRS_RES_WINERR;					// Suppose the task is OK
	...
	if (nResult != WAIT_FAILED){
		...
		if(pQue->nCnt < pQue->nMax){
			...
			// 设置服务状态为队列存在待发送数据项
			g_Devs[Handle - 1].pWork->DrvStat |= BHTRS_ST_SNDQUE;
			// 离开工作空间操作同步临界区
...
@retval     BHTRS_RESULT  BHTRS_RES_WINERR:失败   BHTRS_RES_OK:成功
...
BHTRS_RESULT WaitSendComplete(IN HANDLE hEvent)
{
	BHTRS_RESULT nRes = BHTRS_RES_WINERR;					   //Suppose the task is fail;
...
BHTRS_RESULT PostRecvRequest(IN BHTRS_HANDLE Handle,IN RECVITEM* pItem)
{
	BHTRS_RESULT nRes = BHTRS_RES_WINERR;
...
DLLIMP void BHTrs_ShowLastError(HWND hParent)
{
	...
	::MessageBoxA(hParent, lpMsgBuf, "BHLib", 0);
	::LocalFree(lpMsgBuf);
}
